            ...
            <div class="content">
                ...
                <div class="grid grid-cols-12 gap-6 mt-5">
                    <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
                        <button type="button" onclick="window.location.href='{{ route('addUser') }}'" class="button text-white bg-theme-1 shadow-md mr-2">Create User</button>
                        <div class="dropdown">
                        ...
                        <table class="table table-report -mt-2">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-center whitespace-nowrap">LAST LOGIN AT</th>
                                    ...
                                    <td class="w-40">
                                        <div class="flex">
                                            <div class="w-10 h-10 image-fit zoom-in">
                                                <img alt="Midone Tailwind HTML Admin Template" class="tooltip rounded-full" src="{{ asset('images/userPhoto/' . $users->link) }}" title="Uploaded at 20 November 2021">
                                            </div>
                                        </div>
                                    </td>
                                    <td>
                                        <a href="{{ route('userDetails', $users->id) }}" class="font-medium whitespace-nowrap">{{ $users->first_name }} {{ $users->last_name }}</a>
                                        <div class="text-gray-600 text-xs whitespace-nowrap mt-0.5">{{ $users->email }}</div>
                                    </td>
                                    <td class="text-center">Not logged in yet</td>
                                    <td class="w-40">
                                        <div class="flex items-center justify-center text-theme-9"> <i data-feather="check-square" class="w-4 h-4 mr-2"></i> Active </div>
                                    </td>
                                    <td class="text-center">{{ $users->role->role_name }}</td>
                                    <td class="table-report__action w-56">
                                        <div class="flex justify-center items-center">
                                            <a class="flex items-center mr-3" href="{{ route('editUseer', $users->id) }}"> <i data-feather="check-square" class="w-4 h-4 mr-1"></i> Edit </a>
                                            @if($users->role_id == 2)
                                            <a class="flex items-center text-theme-6" href="{{ route('deleteUser', $users->id) }}"> <i data-feather="trash-2" class="w-4 h-4 mr-1"></i> Delete </a>
                                            @endif
                                        </div>
                                    </td>
                                </tr>
